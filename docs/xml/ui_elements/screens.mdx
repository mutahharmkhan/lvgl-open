...
- <api> <xml_api>: Screens are always created as they are, with no parameters.
- <preview> <xml_preview>: In LVGL's UI Editor, `<preview>` is used to set the style, size, and other parameters of the previewing "screen". For Screens, the
  preview options are defined in `project.xml`. Learn more in [Screen preview](/xml/ui_elements/screens)

## Usage

Each XML file describes a [Screen](/common-widget-features/screens). The name of the XML file will
...
This property affects the [screen load and create events](/xml/ui_elements/events).
...
Screens don't support the <preview> <xml_preview> tag because it doesn't make
...
`<display>` elements can be defined. In the UI   Editor, when a Screen is being developed,
...
Learn more on the [XML Events](/xml/ui_elements/events) page.
