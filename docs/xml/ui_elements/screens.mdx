...
- `<api>`: Screens are always created as they are, with no parameters.
- `<preview>`: In LVGL's UI Editor, `<preview>` is used to set the style, size, and other parameters of the previewing "screen". For Screens, the
