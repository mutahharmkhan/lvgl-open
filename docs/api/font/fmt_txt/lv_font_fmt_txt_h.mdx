...
| `kern_scale` | <ApiLink name="uint16_t" /> | Store kerning values. Can be <ApiLink name="lv_font_fmt_txt_kern_pair_t" />` * or `lv_font_kern_classes_fmt_txt_t *` depending on `kern_classes` */ const void * kern_dsc;<br/><br/>/** Scale kern values in 12.4 format |
