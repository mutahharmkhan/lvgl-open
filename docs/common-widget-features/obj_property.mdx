...
description: "Widget Properties provides a way to greatly reduce the size of the interface between LVGL and whatever logic layer is just above it, to get and set the most important properties of Widgets."
---
...
A Widget's properties are the combined set of [Styles](/common-widget-features/styles) plus additional properties
...
Label Widgets.  A Widget's [local styles](/common-widget-features/styles/local_styles) are also valid
...
<ApiLink name="LV_USE_OBJ_PROPERTY" /> to `1` in `lv_conf.h`.

The 3 functions that then become available are:

- <ApiLink name="lv_result_t" />  <ApiLink name="lv_obj_set_property" display="lv_obj_set_property(widget, lv_property_t * value)" />
  Sets specified property of Widget.
- <ApiLink name="lv_property_t" />  <ApiLink name="lv_obj_get_property" display="lv_obj_get_property(widget, lv_prop_id_t id)" />
  Reads property value from Widget.
- <ApiLink name="lv_result_t" />  <ApiLink name="lv_obj_set_properties" display="lv_obj_set_properties(widget, lv_property_t * values, count)" />
  Sets multiple Widget properties from an array of <ApiLink name="lv_property_t" />.

An `lv_prop_id_t` is a [Property ID](/common-widget-features/obj_property), whereas an `lv_property_t` is a
struct that pairs a [Property ID](/common-widget-features/obj_property) with a [Property Value](/common-widget-features/obj_property).

The following is an example of an array that could be used as the `values` argument
in <ApiLink name="lv_obj_set_properties" />:
...
Alternately, <ApiLink name="lv_obj_set_property" display="lv_obj_set_property(widget, value)" /> could be called using
this array's individual `value` elements inside a loop.

### Property ID

<ApiLink name="lv_prop_id_t" /> identifies which property to get/set.  It is an enum value
...
- <ApiLink name="LV_PROPERTY_ID" /> (for single values -- see Single Values below)`;
  bits `&lt;31:28>` contain the property's value type and bits `&lt;27:0>` contain the
  property ID.
- <ApiLink name="LV_PROPERTY_ID2" /> (for paired values -- see Paired Values below)`;
...
Note that <ApiLink name="lv_style_prop_t" /> (enumerator values beginning with `LV_PROPERTY_STYLE_...`)
are also valid property IDs, and can be used to set or get a Widget's style values.

### Property Value

<ApiLink name="lv_property_t" /> is a struct that begins with an `id` field whose meaning
...
You can find the current <ApiLink name="lv_property_t" /> struct in the
[lv_obj_property.h](https://github.com/lvgl/lvgl/blob/master/src/core/lv_obj_property.h) file.

### Property ID Lookup by Name

Setting configuration macro <ApiLink name="LV_USE_OBJ_PROPERTY_NAME" /> to `1` enables the
following functions to look up property IDs by passing property name (a string):

- <ApiLink name="lv_prop_id_t" /> <ApiLink name="lv_obj_property_get_id" display="lv_obj_property_get_id(widget, name)" />
...
- <ApiLink name="lv_prop_id_t" /> <ApiLink name="lv_obj_class_property_get_id" display="lv_obj_class_property_get_id(class_p, name)" />
  Gets property ID by doing a non-recursive search for `name` directly in Widget
  class properties.
- <ApiLink name="lv_prop_id_t" /> <ApiLink name="lv_style_property_get_id" display="lv_style_property_get_id(name)" />
...
<ApiLink name="lv_obj_property_get_id" display="lv_obj_property_get_id(widget, name)" />, if a name does exist in both lists,
...
For the `lv_property_t * value` argument of the <ApiLink name="lv_obj_set_property" />
...
<ApiLinkList items={["lv_obj_property_names"]} />
