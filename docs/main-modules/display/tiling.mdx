...
description: "When multiple CPU cores are available and a large area needs to be redrawn, LVGL must identify independent areas that can be rendered in parallel."
---
...
<ApiLink name="lv_display_set_tile_cnt" display="lv_display_set_tile_cnt(disp, cnt)" />. The default value is
<ApiLink name="LV_DRAW_SW_DRAW_UNIT_CNT" /> (or 1 if software rendering is not enabled).
...
For example, in <ApiLink name="LV_DISPLAY_RENDER_MODE_DIRECT" /> mode on an 800x480
...
In <ApiLink name="LV_DISPLAY_RENDER_MODE_DIRECT" />, the screen-sized draw buffer is
...
In <ApiLink name="LV_DISPLAY_RENDER_MODE_FULL" />, the maximum number of tiles is
always created when the entire screen is refreshed.

In <ApiLink name="LV_DISPLAY_RENDER_MODE_PARTIAL" />, the partial buffer is divided
...
Tiled rendering only affects the rendering process, and the [Flush Callback](/main-modules/display/setup) is
