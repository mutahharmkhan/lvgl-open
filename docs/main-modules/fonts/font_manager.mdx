---
title: Font Manager
description: "The font manager is a secondary encapsulation of the LVGL font engine, which facilitates the use and management of fonts for applications."
---
...
<ApiLink name="LV_USE_FONT_MANAGER" /> macros to non-zero
values, and configure <ApiLink name="LV_FONT_MANAGER_NAME_MAX_LEN" /> to set the maximum
length of the font name.

### Initialize Font Manager

Use <ApiLink name="lv_font_manager_create" /> to create a font manager, where the
<ApiLink name="recycle_cache_size" /> parameter is used to set the number of font recycling
caches, which can improve font creation efficiency.

Use <ApiLink name="lv_font_manager_add_src_static" /> to add a mapping between font names
...
(for example, allocated from a local variable), use <ApiLink name="lv_font_manager_add_src" /> to add the resource.
This function will copy the contents of the structure itself.

Use <ApiLink name="lv_font_manager_remove_src" /> to remove the font resource mapping.
...
such as <ApiLink name="lv_freetype_font_create" /> and <ApiLink name="lv_tiny_ttf_create_file" />.
...
Use <ApiLink name="lv_font_manager_delete_font" /> to delete the font when it is no longer needed.
...
Use <ApiLink name="lv_font_manager_delete" /> to destroy the entire font manager. It should
be noted that before destruction, it is necessary to ensure that the application has
deleted all fonts using <ApiLink name="lv_font_manager_delete_font" />.  The font manager
...
<ApiLinkList items={["lv_font_manager", "lv_font_manager_recycle"]} />
