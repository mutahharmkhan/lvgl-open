---
title: "File System (lv_fs_drv)"
description: "LVGL has a \"File system\" abstraction module that enables you to attach any type of file system. A file system is identified by an assigned identifier letter."
---
...
under [Identifier Letters](/main-modules/fs).

<Callout type="info">
If you want to skip the drive-letter prefix in Unix-like paths, you can use the
<ApiLink name="LV_FS_DEFAULT_DRIVER_LETTER" /> config parameter.
</Callout>

## Ready-to-Use Drivers

LVGL contains prepared drivers for the API of POSIX, standard C,
Windows, and [FATFS](http://elm-chan.org/fsw/ff/00index_e.html).
Learn more [here](/libs/fs_support/fs).
...
or the character '/'.  See [Adding a Driver](/main-modules/fs) for how this is done.
...
To add a driver, a <ApiLink name="lv_fs_drv_t" /> object needs to be initialized and
registered in a way similar to the code below.  The <ApiLink name="lv_fs_drv_t" /> variable
...
`mode` can be <ApiLink name="LV_FS_MODE_WR" /> or <ApiLink name="LV_FS_MODE_RD" /> to open for writes or reads.
...
[lv_fs_template.c](https://github.com/lvgl/lvgl/blob/master/examples/porting/lv_port_fs_template.c).
...
- <ApiLink name="LV_USE_FS_FATFS" />
- <ApiLink name="LV_USE_FS_STDIO" />
- <ApiLink name="LV_USE_FS_POSIX" />
- <ApiLink name="LV_USE_FS_WIN32" />
- <ApiLink name="LV_USE_FS_MEMFS" />
- <ApiLink name="LV_USE_FS_LITTLEFS" />
- <ApiLink name="LV_USE_FS_ARDUINO_ESP_LITTLEFS" />
- <ApiLink name="LV_USE_FS_ARDUINO_SD" />
...
The mode in <ApiLink name="lv_fs_open" /> can be <ApiLink name="LV_FS_MODE_WR" /> to open for
writes only, <ApiLink name="LV_FS_MODE_RD" /> for reads only, or
<ApiLink name="LV_FS_MODE_RD" /> `|` <ApiLink name="LV_FS_MODE_WR" /> for both.
...
[Image](/widgets/image) Widgets can be opened from files as well (besides
...
is <ApiLink name="LV_FS_SEEK_END" />, in which case `seek` and `tell` will be called
...
<ApiLinkList items={["lv_fsdrv", "lv_fs", "lv_fs_private"]} />
