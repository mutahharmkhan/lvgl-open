...
enable the file's content and set the <ApiLink name="LV_COLOR_DEPTH" /> define to align with
...
1. Set the <ApiLink name="LV_CONF_INCLUDE_SIMPLE" /> define to your compiler
...
1. Set a custom path via the <ApiLink name="LV_CONF_PATH" /> define.
...
define <ApiLink name="LV_CONF_SKIP" /> as a compiler option. Unset options will get a
...
[Kconfig file](https://github.com/lvgl/lvgl/blob/master/Kconfig) is
...
<ApiLink name="lv_display_get_inactive_time" /> can also be used to go to sleep when
...
and provide a custom function to <ApiLink name="lv_global_default" /> using thread` or
