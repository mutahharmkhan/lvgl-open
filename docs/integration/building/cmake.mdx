---
title: CMake
description: "CMake is a cross-platform build system generator. It is used to easily integrate a project/library into another project."
---
...
| --- | --- | --- |
| LV_BUILD_CONF_PATH | PATH | Allows to set a custom path for `lv_conf.h` |
...
| LV_BUILD_DEFCONFIG_PATH | PATH | Specify to use a .defconfig file instead of the current .config in a Kconfig setup. |
...
| CONFIG_LV_BUILD_DEMOS | BOOLEAN | When enabled builds the demos |
| CONFIG_LV_BUILD_EXAMPLES | BOOLEAN | When enabled builds the examples |
| CONFIG_LV_USE_THORVG_INTERNAL | BOOLEAN | When enabled the in-tree LVGL version of ThorVG is compiled |
| CONFIG_LV_USE_PRIVATE_API | BOOLEAN | When enabled the private headers `*_private.h` are installed on the system |
