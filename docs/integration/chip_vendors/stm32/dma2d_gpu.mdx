---
title: STM32 DMA2D
description: "STM32 DMA2D is also called Chrom-ART. It will be referred to as DMA2D on this page."
---
...
1. An RTOS/OS must be present. Set <ApiLink name="LV_USE_OS" /> to one of the supported values
   in `lv_conf_template.h` corresponding to the RTOS you're using.
2. Set <ApiLink name="LV_USE_DRAW_DMA2D_INTERRUPT" /> to `1` in `lv_conf.h`.
3. You must call
   <ApiLink name="lv_draw_dma2d_transfer_complete_interrupt_handler" />
...
<ApiLinkList items={["lv_draw_dma2d", "lv_draw_dma2d_private"]} />
