---
title: glTF
description: "glTF (GL Transmission Format) is a royalty-free specification for efficient transmission and loading of 3D scenes and models."
---
...
Set <ApiLink name="LV_USE_GLTF" /> to `1` in `lv_conf.h`.
...
Follow the OpenGL ES driver setup documentation ([OpenGL](/integration/embedded_linux/drivers/opengl_driver)) to configure a driver that supports OpenGL for your platform.
...
Property values can only be read from within the <ApiLink name="LV_EVENT_VALUE_CHANGED" />
...
<Callout type="warn">
- The <ApiLink name="LV_EVENT_VALUE_CHANGED" /> event fires only when node
  properties are modified, not every frame.
- Getter functions return <ApiLink name="LV_RESULT_INVALID" /> if called outside
  the event callback, ensuring you never read stale data.
- Calling <ApiLink name="lv_gltf_model_node_get_world_position" /> without world
  position registration will return <ApiLink name="LV_RESULT_INVALID" />.
</Callout>

<Callout type="info">
When using the root node, the local position will equal the world position, meaning
you can subscribe to <ApiLink name="lv_gltf_model_node_add_event_cb" /> instead of
<ApiLink name="lv_gltf_model_node_add_event_cb_with_world_position" /> and use
<ApiLink name="lv_gltf_model_node_get_local_position" /> to get the world position without
...
- <ApiLink name="LV_GLTF_ANIM_SPEED_NORMAL" /> equals 1000, representing 1.0x (normal) speed
...
<ApiLinkList items={["lv_gltf_data_internal", "lv_gltf_model", "lv_gltf_model_node", "lv_gltf_environment", "lv_gltf_environment_private", "lv_gltf_view_shader", "lv_gltf", "lv_gltf_view_internal", "lv_gltf_math.hpp"]} />
