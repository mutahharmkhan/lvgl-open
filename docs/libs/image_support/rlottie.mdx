---
title: Rlottie Player
description: "The Rlottie animation player for LVGL is a 3rd-party extension for LVGL that allows playing Lottie animations in LVGL. It provides an interface to Samsung/rlottie library's C API."
---

<Callout type="warn">
Rlottie is deprecated.  Consider using the [Lottie (lv_lottie)](/widgets/lottie) Widget instead.
</Callout>
...
<ApiLink name="lv_alloc" display="lv_alloc/lv_realloc" /> calls in `lv_rlottie.c` with <ApiLink name="heap_caps_malloc" /> (for
...
<ApiLinkList items={["lv_rlottie", "lv_rlottie_private"]} />
